...
export class LightenerCurveCardEditor extends LitElement {
  ...
  private _ensurePickerLoaded(): void {
    ...
    const kickLoaders = async () => {
      try {
        const loadHelpers = (window as unknown as { loadCardHelpers?: () => Promise<unknown> }).loadCardHelpers;
        ...
        <div class="field">
          ...
          ${this._pickerReady
            ...
            : html`
                ...
                />
                <span class="hint">
                  Entity picker unavailable — enter a Lightener light entity ID manually
                  (must start with <code>light.</code>).
                </span>
              `}
        </div>
        <div class="field">
